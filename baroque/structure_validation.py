...
def parse_metadata_export(metadata_export, level):
    ...
    # File type: xlsx
        ...
        if target_col_letter == "":
            print("SYSTEM ERROR: metadata export does not contain 'DigFile Calc' column")
            sys.exit()
...
def check_empty_directory(baroqueproject, directory_path):
    if len(os.listdir(directory_path)) == 0:
        baroqueproject.add_errors(
            "structure",
            "requirement",
            directory_path,
...
def check_empty_file(baroqueproject, file_path):
...
def validate_directory(baroqueproject, metadata_export, level):
    ...
    for path in process_paths:
        check_empty_directory(baroqueproject, path)
...
def check_item_files(baroqueproject):
    ...
    for item in baroqueproject.items:
        ...
        check_empty_file(baroqueproject, item["path"])
...
def check_intellectual_groups_numbers(baroqueproject):
        """
        ...
        for item in items.keys():
            ...
            for item_dict in baroqueproject.items:
...
def check_intellectual_groups_files(baroqueproject):
    """
...
def validate_file(baroqueproject):
    ...
    check_intellectual_groups_numbers(baroqueproject)
    check_intellectual_groups_files(baroqueproject)
